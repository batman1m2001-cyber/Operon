...
def unpack(item=None) -> dict:
    """Payload → the fields the agent consumes."""
    item = item if isinstance(item, dict) else {}
    return {"query": item.get("query", "What is 25 * 4 + 100?")}
...
def main_flow():
    ...
    runner = agent(query=fields["query"])
    out = egress(item=runner["answer"])
    START >> request >> fields >> runner >> out >> END
