...
def unpack(item=None) -> dict:
    """Payload → the fields the pipelines consume."""
    item = item if isinstance(item, dict) else {}
    return {
        "text": item.get("text", "streaming delivers tokens as they are made"),
        "chunk_size": item.get("chunk_size", 3),
        "n": item.get("n", 4),
    }
...
def main_flow():
    ...
    chunks = text_pipeline(text=fields["text"], chunk_size=fields["chunk_size"])
    squares = async_pipeline(n=fields["n"])
    out = egress(item=squares["label"])
    START >> request >> fields >> chunks >> squares >> out >> END
