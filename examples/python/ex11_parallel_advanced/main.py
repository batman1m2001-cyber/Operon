...
# One main graph, served: ingress in, the three parallel patterns
...
def unpack(item=None) -> dict:
    ...
    return {
        "text": item.get("text", "This is a great product with good quality"),
        "items": item.get("items", [1, 2, 3, 4, 5]),
    }
...
def main_flow():
    ...
    fanned = fan_out(text=fields["text"])
    each = iteration(items=fields["items"])
    tolerant = partial_failure(items=fields["items"])
    out = egress(item=tolerant["results"])
    START >> request >> fields >> fanned >> each >> tolerant >> out >> END
