...
def unpack(item=None) -> dict:
    """Payload → the question the RAG flow answers."""
    item = item if isinstance(item, dict) else {}
    return {"question": item.get("question", "What is operonx?")}
...
def main_flow():
    ...
    answered = rag(question=fields["question"])
    out = egress(item=answered["answer"])
    START >> request >> fields >> answered >> out >> END
