...
def unpack(item=None) -> dict:
    """Payload → the fields the pipelines below consume."""
    item = item if isinstance(item, dict) else {}
    return {"text": item.get("text", SAMPLE_TEXT)}
...
def main_flow():
    ...
    numbers = data_pipeline()
    words = text_pipeline(text=fields["text"])
    out = egress(item=words["report"])
    START >> request >> fields >> numbers >> words >> out >> END
