...
def unpack(item=None) -> dict:
    ...
    return {
        "question": item.get("question", "Xin chào!"),
        "text": item.get("text", SAMPLE_TEXT),
    }
...
def main_flow():
    ...
    chat = basic_chat(question=fields["question"])
    summary = summarize_pipeline(text=fields["text"])
    out = egress(item=summary["content"])
    START >> request >> fields >> chat >> summary >> out >> END
