...
# One main graph, served: ingress in, the three techniques composed as
# nested graphs, egress out.
...
def unpack(item=None) -> dict:
    ...
    return {
        "text": item.get("text", "Sản phẩm này tuyệt vời!"),
        "query": item.get("query", "Tính 25 * 17 giúp tôi"),
        "message": item.get("message", "Xin chào!"),
    }
...
def main_flow():
    ...
    sentiment = structured_output(text=fields["text"])
    tools = tool_calling(query=fields["query"])
    turn = multi_turn(history=[], message=fields["message"])
    out = egress(item=turn["new_history"])
    START >> request >> fields >> sentiment >> tools >> turn >> out >> END
